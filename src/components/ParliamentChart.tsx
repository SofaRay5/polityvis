...
import { countryName, countryPartyName } from '../lib/countryDisplay'
...
export function ParliamentChart({ country, locale }: { country: Country; locale: Locale }) {
  ...
  const title = `${countryName(country, locale)} ${t['navigation.parliament']}`
  return <section className="parliament" aria-labelledby="parliament-title">
    ...
    <ul className="party-legend">{orderedGroups.map((group) => <li key={group.id}><span className="party-swatch" style={{ background: group.color }} />{countryPartyName(country, group, locale)}<small>({group.ideologyPosition > 0 ? '+' : ''}{group.ideologyPosition})</small><b>{group.seats}</b></li>)}</ul>
  </section>
}
